...
function VariantForm({ variant, onSave, onDelete, saving, deleting }) {
    ...
    <form onSubmit={handleSubmit} className="mb-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800">
      <div className="mb-3 flex items-center justify-between">
        ...
        {!isNew && (
          <button
            ...
            onClick={onDelete}
            disabled={deleting}
            className="rounded-lg p-1 text-slate-400 hover:bg-rose-50 hover:text-rose-500 disabled:opacity-40"
          >
      ...
      <div className="flex flex-col gap-3">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 self-end rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-60"
        >
...
export default function StepEditorPanel({ step, workspaceId, campaignId, onClose, onSaved }) {
  ...
  async function handleSaveExisting(emailId, data) {
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        throw new Error(err.detail || "Failed to save variant.");
      }
    ...
    <div className="flex h-full flex-col overflow-hidden border-l border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-900">
      ...
      <div className="flex-1 overflow-y-auto px-5 py-5">
        <p className="mb-4 text-xs text-slate-500">
          ...
          Use <code className="rounded bg-slate-100 px-1 py-0.5 font-mono text-[11px]">{"{{first_name}}"}</code> for merge tags.
        </p>
        ...
        {showNewForm ? (
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-xl border-2 border-dashed border-slate-200 py-3 text-sm font-medium text-slate-400 transition-colors hover:border-blue-300 hover:text-blue-500 dark:border-slate-700 dark:hover:border-blue-600"
          >
