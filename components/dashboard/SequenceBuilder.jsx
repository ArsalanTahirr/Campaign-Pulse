...
function StepCard({ step, workspaceId, campaignId, onEdit, onDelete, deleting }) {
        ...
        <div className="flex items-center gap-1">
          <PermissionGate action="manage_sequence">
            <button
              type="button"
              onClick={() => onEdit(step)}
              className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 hover:text-blue-600 dark:hover:bg-slate-700"
            ...
            <button
              ...
              className="rounded-lg p-1.5 text-slate-400 hover:bg-rose-50 hover:text-rose-500 disabled:opacity-40 dark:hover:bg-slate-700"
...
function AddStepForm({ nextStepNumber, workspaceId, campaignId, onAdded, onCancel }) {
    ...
    <form onSubmit={handleSubmit} className="rounded-xl border-2 border-dashed border-blue-200 bg-blue-50/40 p-4 dark:border-blue-900 dark:bg-slate-800/50">
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-slate-600">Send time (HH:MM)</label>
            <input
              type="time"
              value={sendTime}
        ...
        <div>
          <label className="mb-1.5 block text-xs font-medium text-slate-600">Send days (optional)</label>
...
export default function SequenceBuilder({ workspaceId, campaignId }) {
